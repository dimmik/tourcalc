...
`target/server/`. Отличие от `release` одно: `panic = "unwind"`. Общий профиль режет
раскрутку ради размера wasm, а сервер с ним от паники в одном обработчике падал целиком;
теперь `CatchPanicLayer` отвечает на такой запрос 500 и работает дальше.
...
├── news.rs          (реэкспорт tc_core::news) текст push-уведомления человеческими словами
...
│   ├── auth.rs      /api/Auth/*
│   ├── tour.rs      чтение; /api/Tour/{id}/state — только StateGUID, для «кто-то поменял тур»
...
├── list.rs        список туров, вход, создание; порядок (новые / тронутые / имя /
│                  потрачено, направление переключается повторным нажатием, выбор
│                  помнится на устройстве)
...
├── dialogs.rs     формы: трата, участник, тур, валюты, версии. Esc = Cancel; начатая
│                  и незакрытая форма остаётся на устройстве (drafts.rs)
├── drafts.rs      что набрали в форме и не сохранили: два часа хранится, подставляется
│                  в следующую пустую форму того же тура, «Start blank» стирает
├── edit.rs        черновики форм и как они меняют тур
├── queue.rs       операции, ожидающие отправки; localStorage. Отказы сервера: после трёх
│                  подряд очередь не шлётся сама, тур предлагает Try again / Discard.
│                  Правка того, что кто-то удалил, отбрасывается (и об этом говорится)
├── sync.rs        отправка очереди и разрешение конфликтов; fetch — прочитать, ничего не отправляя
├── others.rs      чужие правки открытого тура: опрос state (при возврате во вкладку и раз в N с — Settings, по умолчанию 10),
│                  подмена тура / плашка в открытой форме / подсветка изменённых трат
├── api.rs         HTTP к серверу. Ошибка запроса — `Failed { why, said }`: не достучались,
│                  сервер ответил кодом (4xx — отказ, 5xx — сервер сломался) или мы не
│                  поняли ответ. Строка свежести говорит это разными словами
...
├── version.rs     точка в шапке (зелёная — последняя, янтарная — есть новее, серая —
│                  сервер не ответил; при открытии на 3 с разворачивается в слово),
│                  полоска с кнопкой Update и страница сборки в Help
...
**Возврат долга — не трата.** Возвраты и переводы внутри семьи хранятся как траты (так
приложение записывает «Паша отдал Вале 47 026»), но отличаются пустой категорией и ни в
какую сумму расходов не входят — иначе тур дорожал бы от каждого расчёта. Правило живёт в
`tc_core::is_an_expense`, сумма — `tc_core::spent_on_expenses`; спрашивать надо их, а не
писать условие заново. Список туров сумму не считает: у него нет трат (сервер их вырезает),
и цифру присылает сервер полем `TotalSpentInCents`. Тем же путём приходит `LeftToSettleInCents`
— сколько ещё передать друг другу; считать это по `SuggestedPaymentsCount` нельзя: деление в
целых копейках оставляет крошки, и у рассчитавшегося тура платежей всё равно шесть.

**Тур без `StateGUID`.** Туры 2022 года писались до того, как появилось это поле, и в
документе его просто нет. Мягкая блокировка в Mongo — это фильтр `{ StateGUID: <что читали> }`,
а отсутствующее поле **не равно** пустой строке: фильтр не совпадал, запись читалась как
«кто-то успел раньше», и сохранение такого тура вечно отвечало 409. Теперь пустое ожидание
совпадает только с документом, у которого состояния нет ни в одном написании (`still_at` в
`mongo.rs`); первое же сохранение записывает `StateGUID`, и дальше всё как обычно.

**`Vary` и кэш service worker'а.** `caches.match(request)` учитывает `Vary` ответа и
сравнивает заголовки, которые браузер пишет сам, а JS их не видит (`accept-encoding`,
`origin`). Сервер сжимает ответы и зеркалит CORS, поэтому `Vary` есть у всего — и промах
кэша выглядел как «файла там нет»: wasm лежал в кэше и всё равно качался при каждом старте,
на медленной сети это полминуты и экран «Starting…». Лечится `{ ignoreVary: true }` (см.
`kept()` в `sw.js`): файлы сборки названы по своему содержимому, выбирать между вариантами
там не из чего.
...
Сборка с `BUILD_TYPE` не `na` (то есть любая из CI) на нём **не стартует** и пишет в лог
почему; если это правда нужно — `AllowDevelopmentKey=true`.
